            ...
            >
              <div className="w-full h-full bg-gray-200 rounded-2xl overflow-hidden shadow-xl group-hover:shadow-2xl transition-shadow duration-300">
                <div className="w-full h-full bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center relative">
                 <Image
  src={`/stock/${pose.link}`}
  width={192}       // exactly the tailwind width: w-48 => 12rem => 192px
  height={256}      // h-64 => 16rem => 256px
  alt={`${pose.link}`}
  className="object-cover w-full h-full" // ensure it fills the container
  quality={90}      // optional, defaults lower — raise if you want crisper
  priority={index === 2} // optional: set priority for the main image
/>
                </div>
              </div>
            </div>
